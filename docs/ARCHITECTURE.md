...
и Playwright E2E. Production-аутентификация, долговременный прогресс, выдача наград,
полные экраны профиля/лидерборда и доставка уведомлений остаются будущей работой.

## Компоненты и направление зависимостей

```mermaid
flowchart LR
    React[React Scenario Runner] --> Client[API client and recovery]
...
Хранение уведомлений и автоматическая выдача наград — будущая работа.
...
`(employee_id, achievement_id, achievement_version)`; автоматическая выдача
остаётся будущим этапом. API v1 получает employee_id из demo-токена и проверяет
...
и разблокировки читаются из отдельных таблиц без автоматического начисления.
