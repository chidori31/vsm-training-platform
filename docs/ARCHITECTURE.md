...
HTTP API прохождения, хранение сессий в БД, UI игры, выдача наград
...
flowchart LR
    ...
    Snapshots[Pydantic session snapshots] --> Engine
    UseCases -. future .-> SQL
...
начальные и текущие ScoreState, started_at, status, completed_at и упорядоченные Decision.
Decision хранит id, session_id, node_id, choice_id, порядковый номер, decided_at
и применённые эффекты с explanation. Проверяются принадлежность решений, уникальность ID,
...
loyalty, safety и объявленные компетенции; значений по умолчанию нет.
...
JSON-снимка и функции `dump_session`/`load_session`. Сериализация и загрузка
проверяют снимок движком. Сохранение строки выполняет вызывающая сторона;
хранилища сессий этот адаптер не создаёт. Проверка истории подтверждает
согласованность, но не авторизацию или подлинность клиентского снимка.
...
версия записи должна защищать от параллельных решений, decision_id — от повторной
доставки на границе БД. Чистая проверка expected_sequence не заменяет атомарное
сравнение ревизии при сохранении. Уникальность AchievementUnlock — `(employee_id, achievement_id)`;
правило выдачи версионируется. События аналитики и уведомления публикуются
после успешного commit через outbox. Порты, транзакции решений, уникальность ID
решений в БД, outbox и доставка описаны, но не реализованы. Транзакционный импорт
...
2. Проверить альтернативные ветки, условия до и после эффектов, неверные
   переходы, повторы, устаревшие команды, таймеры и восстановление движком.
