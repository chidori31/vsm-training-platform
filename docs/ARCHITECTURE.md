...
Текущий scope проводника подтверждён. `domain/shifts.py` формирует воспроизводимый
маршрут, агрегирует проверенные сессии и определяет учебные квалификации.
`application/shifts.py` сохраняет маршрут и командные квитанции; следующий этап
открывается только после завершения закреплённой попытки. Существующий
SessionService сохраняет авторитет времени, состояния и XP. Итог смены суммирует
session_rewards и не выдаёт повторную награду. 07 хранит смены/этапы/квалификации.

08 хранит атомарный аудит принятых команд и независимые счётчики лимитов PostgreSQL.
DomainEvent переиспользуется для серверного факта аудита, но offline-claim
не становится доверенным игровым решением. Общий EARN reducer остаётся отдельным
контрактом; учебные награды сохраняют существующий durable ledger.

```mermaid
flowchart LR
  UI[Маршрут и разбор] --> HTTP[FastAPI /api/v1/shifts]
  HTTP --> Shift[ShiftService]
  Shift --> Plan[Чистый план по seed]
  Shift --> Session[SessionService]
  Session --> Engine[Чистый scenario engine]
  Session --> XP[session_rewards]
  Session --> Audit[Аудит команд]
  HTTP --> Rate[Лимиты команд]
  Shift --> PG[(PostgreSQL)]
  XP --> PG
  Audit --> PG
  Rate --> PG
...
Нет нового источника истины в React: клиент хранит только токен/намерение,
загружает маршрут и повторно читает актуальное состояние попытки.
[SHIFT](SHIFT.md) · [ANTI_CHEAT](ANTI_CHEAT.md).
