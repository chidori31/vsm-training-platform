...
Текущий этап подтверждён владельцем: внутренний учебный сервис проводника.
Пассажирская loyalty-система не разрабатывается. Историческая QA-сверка
сохранена в [MEETUP_ALIGNMENT](docs/MEETUP_ALIGNMENT.md).
...
Главная страница предлагает учебную смену из трёх событий: сервис → конфликт →
критическая ситуация. Сервер выбирает и сохраняет воспроизводимый маршрут.
Стандартная и сложная смены отличаются ветвями и сроками решения. Итог объединяет
профессиональные показатели, квалификации и раскрываемый разбор каждого события.
Личная сводка содержит последние результаты, задачу по слабому навыку и динамику.
[Механика смены](docs/SHIFT.md) · [Anti-cheat](docs/ANTI_CHEAT.md) ·
[Визуальная система](docs/SERVICE_DESIGN.md).

Пять базовых сценариев, 12 вариантов событий смены и пять учебных персон синтетические. Баллы условны;
...
python -m app.scenarios import ../scenarios/shifts
...
Общие demo identities, отсутствие production SSO/RBAC/TLS,
